...
extern u8 BattleScript_FriskActivate[];
    ...
    while (gBattleStruct->turnEffectsBank < gBattlersCount && gBattleStruct->turnEffectsTracker <= TURNBASED_MAX_CASE)
    {
        ...
        else
        {
            ...
            case 18:  // item effects again
                ...
                break;
            case 3:  // leech seed
                if (gStatuses3[gActiveBattler] & STATUS3_LEECHSEED && gBattleMons[gStatuses3[gActiveBattler] & STATUS3_LEECHSEED_BANK].hp != 0 && gBattleMons[gActiveBattler].hp != 0 && gBattleMons[gActiveBattler].ability != ABILITY_MAGIC_GUARD)
                {
                ...
                break;
			case 4:  // poison
                if ((gBattleMons[gActiveBattler].status1 & STATUS_POISON) && gBattleMons[gActiveBattler].hp != 0 && gBattleMons[gActiveBattler].ability != ABILITY_POISON_HEAL && gBattleMons[gActiveBattler].hp != 0 && gBattleMons[gActiveBattler].ability != ABILITY_MAGIC_GUARD)
                {
                    gBattleMoveDamage = gBattleMons[gActiveBattler].maxHP / 8;
                    if (gBattleMoveDamage == 0)
                        gBattleMoveDamage = 1;
...
u8 AtkCanceller_UnableToUseMove(void)
{
    ...
    do
    {
        ...
        case 9: // confusion
            ...
            break;
        case 10: // paralysis
            if (gBattleMons[gBankAttacker].status1 & STATUS_PARALYSIS && (Random() % 4) == 0 && gBattleMons[gBankAttacker].ability != ABILITY_MAGIC_GUARD)
            {
        ...
        case ABILITYEFFECT_ON_SWITCHIN: // 0
            ...
            case ABILITY_FRISK:
                if (gBattleMons[bank ^ 1].item != 0 || gBattleMons[bank ^ 3].item != 0)
                {
                    ...
                    {
                        BattleScriptPushCursorAndCallback(BattleScript_FriskActivate);
                        gBattleStruct->scriptingActive = bank;
                        effect++;
                    }
                }
            ...
            if (gBattleMons[bank].hp != 0)
            {
                ...
                case ABILITY_SHED_SKIN:
                    if ((gBattleMons[bank].status1 & STATUS_ANY) && (Random() % 3) == 0)
                    {
                        ...
                        // BUG: The nightmare status does not get cleared here normally. This is fixed in Emerald and I decided to fix it here too, because why not.
